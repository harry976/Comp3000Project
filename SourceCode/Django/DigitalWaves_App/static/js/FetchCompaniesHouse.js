document.addEventListener("DOMContentLoaded", () => {
    ...
    ScanButton.addEventListener("click", async () => {
        try {
            ...
            const response = await fetch("/FetchCompaniesHouse/"); // Call the view
            ...
            if (data.GoogleResults && data.GoogleResults.length > 0) {
                data.GoogleResults.forEach((result) => {
                    ...
                    if (result.link.includes(".gov.uk")) {
                        DynamicTemplateIcon.src = "/static/CompaniesHouse.png";
                        DynamicTemplateIcon.alt = `CompaniesHouse`;
                    }
                    else {
